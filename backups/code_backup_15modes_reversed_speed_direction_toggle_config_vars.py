...
ENCODER_BRIGHTNESS_ADDR = 0x36  # Default encoder (no jumpers)
ENCODER_COLOR_ADDR = 0x37       # Encoder with A0 jumper
ENCODER_SPEED_ADDR = 0x38       # Encoder with A1 jumper
...
BRIGHTNESS_DEFAULT = 0.25  # Default brightness (0.0 to MAX_BRIGHTNESS)
...
seesaw_brightness = seesaw.Seesaw(i2c, addr=ENCODER_BRIGHTNESS_ADDR)  # Encoder 1: Brightness
seesaw_color = seesaw.Seesaw(i2c, addr=ENCODER_COLOR_ADDR)            # Encoder 2: Color (A0)
seesaw_index = seesaw.Seesaw(i2c, addr=ENCODER_SPEED_ADDR)            # Encoder 3: Speed (A1)

# Verify all encoders
for name, seesaw_dev, addr in [("Brightness", seesaw_brightness, ENCODER_BRIGHTNESS_ADDR),
                                ("Color", seesaw_color, ENCODER_COLOR_ADDR),
...
print("=" * 50)
print("Encoder 1 (0x36): Brightness")
print("Encoder 2 (0x37): Color/Hue")
print("Encoder 3 (0x38): Animation Speed/Index")
print("-" * 50)
print("Button 1: Reverse animation direction")
print("Button 2: Random color")
print("Button 3: Reset to defaults")
print("=" * 50)
...
def update_animation():
    ...
    if ANIMATION_MODE == 0:  # Solid Color
        ...
        for i in range(NUM_PIXELS):
            pixel_hue = ((i * 256 // NUM_PIXELS) + hue_value + (animation_frame * direction)) % 256
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        chase_pos = ((animation_frame * direction) // speed_div) % NUM_PIXELS
        if chase_pos < 0:
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        pulse_phase = (animation_frame * 2 // speed_div) % 512
        if pulse_phase < 256:
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        for i in range(NUM_PIXELS):
            base_intensity = (i * 37 + (animation_frame * 3 // speed_div)) % 256
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        if animation_frame % (speed_div * 2) == 0:
            for i in range(NUM_PIXELS):
        ...
        direction = -1 if reverse_direction else 1
        for i in range(NUM_PIXELS):
            wave_pos = (i * 256 // NUM_PIXELS + (animation_frame * direction // speed_div)) % 256
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        scan_pos = ((animation_frame * direction) // speed_div) % (NUM_PIXELS * 2)
        if scan_pos < 0:
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        pos = ((animation_frame * direction) // speed_div) % (NUM_PIXELS * 2)
        if pos < 0:
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        for i in range(NUM_PIXELS):
            drop_pos = (i * 3 + (animation_frame // speed_div)) % (NUM_PIXELS * 2)
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        for i in range(NUM_PIXELS):
            angle1 = (i * 2 + animation_frame // speed_div) % 256
            angle2 = (i * 3 + animation_frame // speed_div * 2) % 256
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        spiral_pos = ((animation_frame * direction) // speed_div) % NUM_PIXELS
        for i in range(3):
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        bounce_pos = abs(((animation_frame * direction) // speed_div) % (NUM_PIXELS * 2 - 2) - (NUM_PIXELS - 1))
        ...
        speed_div = max(1, animation_speed // ANIMATION_SPEED_DIVISOR)
        direction = -1 if reverse_direction else 1
        comet_pos = ((animation_frame * direction) // speed_div) % NUM_PIXELS
        if comet_pos < 0:
...
while True:
    ...
    # Handle brightness encoder
    if pos_brightness != last_pos_brightness:
        change = pos_brightness - last_pos_brightness
        BRIGHTNESS = max(0.0, min(MAX_BRIGHTNESS, BRIGHTNESS + (change * BRIGHTNESS_CHANGE_RATE)))
    ...
    # Handle button 1: Reverse animation direction
    try:
    ...
    # Handle button 3: Reset to defaults
    try:
    ...
    # Update animation (throttle to ~30 FPS)
    if current_time - last_update >= 0.033:
        update_animation()
